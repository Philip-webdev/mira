...
function Manager() {
  ...
  return (
    <div className="min-h-screen bg-[#0d1a0f] pb-28 text-slate-900" style={{ fontFamily: "Sora, sans-serif" }}>
      <div className="mx-auto max-w-md px-4 py-6">
        <div className="rounded-[2rem] bg-[#0d1a0f] px-4 py-4 shadow-lg">
          <div className="flex items-center justify-between">
            <button
              type="button"
              onClick={() => navigate(-1)}
              className="inline-flex h-11 w-11 items-center justify-center rounded-full bg-emerald-500/10 transition hover:bg-emerald-500/20"
            >
              <ArrowLeft className="h-5 w-5 text-[#5cad1a]" />
            </button>
          ...
          <div className="mt-6 flex flex-col items-center gap-3">
            <label htmlFor="avatar-upload" className="relative flex h-28 w-28 cursor-pointer items-center justify-center rounded-full bg-emerald-100 shadow-sm ring-4 ring-emerald-200/60 transition hover:scale-[1.02] focus:outline-none focus:ring-2 focus:ring-emerald-400">
              {profileForm.avatarDataUrl ? (
                ...
                <div className="flex h-full w-full items-center justify-center rounded-full bg-emerald-100">
                  <UserCircle2 className="h-16 w-16 text-emerald-600" />
                </div>
              )}
              <input id="avatar-upload" type="file" accept="image/*" onChange={handleAvatarChange} className="sr-only" />
              <span className="pointer-events-none absolute bottom-1 right-1 inline-flex h-8 w-8 items-center justify-center rounded-full bg-white text-emerald-700 shadow-sm">
        ...
        <div className="mt-5 rounded-[1.25rem] bg-[#0d1a0f] p-3 shadow-lg">
          <div className="grid grid-cols-2 gap-2 rounded-full bg-emerald-950/70 p-1">
            <button
              type="button"
              onClick={() => setActiveTab("profile")}
              className={`rounded-full px-3 py-2 text-sm font-semibold transition ${
                activeTab === "profile" ? "bg-[#5cad1a] text-white" : "text-slate-300"
              }`}
            >
              Profile
            </button>
            <button
              ...
              className={`rounded-full px-3 py-2 text-sm font-semibold transition ${
                activeTab === "preferences" ? "bg-[#5cad1a] text-white" : "text-slate-300"
              }`}
            >
              Preferences
            </button>
          ...
          {activeTab === "profile" ? (
            <form onSubmit={handleProfileSave} className="mt-4 space-y-4">
              <div className="rounded-[1rem] border border-emerald-500/15 bg-emerald-500/5 p-4">
                <div className="flex items-center justify-between">
                  <div>
                ...
                <div className="mt-4 space-y-3">
                  <label className="block text-sm font-medium text-white">
                    ...
                    <input
                      ...
                      className="mt-2 w-full rounded-xl border border-slate-700 bg-emerald-500/10 px-4 py-3 text-sm text-slate-100 placeholder-slate-500 focus:border-emerald-500 focus:outline-none"
                    />
                  </label>
                  <label className="block text-sm font-medium text-white">
                    Phone number
              ...
              <button
                type="submit"
                disabled={isSaving}
                className="w-full rounded-xl bg-[#5cad1a] px-4 py-3 text-sm font-semibold text-white transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:bg-emerald-300"
              >
            ...
            <div className="mt-4 space-y-3">
              <div className="rounded-[1rem] border border-emerald-500/15 bg-emerald-500/5 p-4">
                ...
                >
                  ...
                  <ChevronDown className={`h-5 w-5 text-emerald-400 transition ${isPreferencesOpen ? "rotate-180" : ""}`} />
                </button>
                  ...
                  <div className="mt-4 space-y-3 border-t border-emerald-500/10 pt-4">
                    <div className="flex items-center justify-between rounded-xl bg-slate-950/60 px-3 py-3">
                      ...
                      <button
                        ...
                        className={`relative h-6 w-11 rounded-full transition ${preferences.notificationsEnabled ? "bg-emerald-500" : "bg-slate-700"}`}
                      >
                    ...
                    <div className="flex items-center justify-between rounded-xl bg-slate-950/60 px-3 py-3">
                      ...
                      <button
                        ...
                        className={`relative h-6 w-11 rounded-full transition ${preferences.secureLogin ? "bg-emerald-500" : "bg-slate-700"}`}
                      >
                    ...
                    <div className="flex items-center justify-between rounded-xl bg-slate-950/60 px-3 py-3">
                      ...
                      <button
                        ...
                        className={`relative h-6 w-11 rounded-full transition ${preferences.privacyMode ? "bg-emerald-500" : "bg-slate-700"}`}
                      >
