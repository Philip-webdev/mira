...
import { GraduationCap, DollarSign, CreditCard, TrendingUp, Users, AlertCircle, Clock, Megaphone, Download, FileText, Search } from "lucide-react";
...
const SslmDashboard = () => {
  ...
  useEffect(() =>{
    ...
    const recentTransactions = async () => {
      const response = await fetch('https://Mira-backend-main.onrender.com/api/admin/dashboard/SSLM');
      const result = await response.json();
      //console.log(result);
      
      setResult(await result.slice(-5));
  };
    recentTransactions();
  ...
  return (
    <DashboardLayout titile="sslm Dashboard" variant="sslm">
      {submitted && (
      ...
      <div className="space-y-8">
        ...
        <div 
  style={{ 
    backgroundImage: 'url("https://i.imgur.com/g0YPRZQ.jpeg")', 
...
>
  ...
  <div className="absolute bottom-6 left-6 z-10 text-white">
    <h2 className="text-2xl font-bold">Soil Science & Land Management (SSLM) </h2>
     ...
     Payment Management dashboard
        ...
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-2">
          {stats.map((stat, index) => (
            <Card key={index} className="bg-card/50 backdrop-blur border-sslm/30">
...
<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card className="bg-card/50 backdrop-blur border-Aqua/30">
            <CardHeader>
              <CardTitle className="text-foreground flex items-center">
                <Clock className="w-5 h-5 mr-2 text-Aqua" />
                Recent Transactions
              </CardTitle>
            </CardHeader>
        ...
        {/* All Payments Table */}
        <Card className="bg-card/50 backdrop-blur border-sslm/30">
          <CardHeader>
            <CardTitle className="text-foreground flex items-center">
              <CreditCard className="w-5 h-5 mr-2 text-sslm" />
              All User Payments
            </CardTitle>
          </CardHeader>
          <CardContent>
