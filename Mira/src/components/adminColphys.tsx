...
import colphyssLogo from '@/assets/WhatsApp Image 2025-09-28 at 8.06.55 PM.jpeg';

const ColphysDashboard = () => {
  ...
  useEffect(() =>{
    setSubmitted(true);
      const recentTransactions = async () => {
        const response = await fetch('https://Mira-backend-main.onrender.com/api/admin/dashboard/COLPHYS');
  ...
  return (
    <DashboardLayout titile="colphyss Dashboard" variant="colphyss">
      {submitted && (
      ...
      <div className="space-y-8">
                <div className="space-y-2">
                   {/* Welcome Section */}
                          ...
                          <div 
                    style={{ 
                      backgroundImage: `url(${colphyssLogo})`, 
                  ...
                  >
                    ...
                    <div className="absolute bottom-6 left-6 z-10 text-white">
                      <h2 className="text-2xl font-bold">College of Physical Sciences (COLPHYS)</h2>
        ...
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-2">
          {stats.map((stat, index) => (
            <Card key={index} className="bg-card/50 backdrop-blur border-colerm/30 h-auto w-auto ">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card className="bg-card/50 backdrop-blur border-colerm/30">
            <CardHeader>
              <CardTitle className="text-foreground flex items-center">
                <Clock className="w-5 h-5 mr-2 text-colerm" />
                Recent Transactions
              </CardTitle>
            </CardHeader>
        ...
        <Card className="bg-card/50 backdrop-blur border-colerm/30">
          <CardHeader>
            <CardTitle className="text-foreground flex items-center">
              <CreditCard className="w-5 h-5 mr-2 text-colerm" />
              All User Payments
            </CardTitle>
          </CardHeader>
          <CardContent>
...
export default ColphysDashboard;